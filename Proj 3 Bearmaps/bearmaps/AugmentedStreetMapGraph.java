...
public class AugmentedStreetMapGraph extends StreetMapGraph {
    ...
    private List<Point> streetIntersection;

    public AugmentedStreetMapGraph(String dbPath) {
        super(dbPath);
        // You might find it helpful to uncomment the line below:
        List<Node> nodes = this.getNodes();
        streetIntersection = new ArrayList<>(27500);
        map = new HashMap<>(37000);

        for (Node n : nodes) {
            Long nodeid = n.id();
            Point current = new Point(projectToX(n.lon(), n.lat()),projectToY(n.lon(), n.lat()));
            map.put(current, nodeid);
            if (neighbors(nodeid).size() > 0) {
                streetIntersection.add(current);
            }
        }
        ktree = new KDTree(streetIntersection);
    }
    ...
    public long closest(double lon, double lat) {
        ...
        return map.get(ktree.nearest(x,y));
    }
